...
fun DisciplinePickerSheet(
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            Text(
                text = stringResource(R.string.settings_label_discipline),
                style = MaterialTheme.typography.headlineSmall,
                color = MaterialTheme.colorScheme.onSurface,
            )
            Text(
                "Used as the default when creating new workouts and combos.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            Spacer(Modifier.height(sp.small))
            Discipline.entries.forEach { disc ->
                OptionItem(
                    label = disc.label(),
                    icon = Icons.Filled.SportsMartialArts,
                    isSelected = disc == current,
                    accentColor = disc.accentColor(),
                    onSelect = { onSelect(disc); onDismiss() },
                )
            }
        }
    }
